...
class NinjaAPI {
  ...
  update(options) {
    ...
    return new Promise(function (resolve, reject) {
      if (!self.updating) {
        self.updating = true
        const promises = self._getRequestCallsArray(
          options.league,
          options.delay
        )
  ...
  _requestApiData(api, league, delay) {
    const self = this
    const url = Helpers.buildApiUrl(api.overview, api.type, league)
  ...
  _processRequest(contents, api, league, resolve, reject) {
    if (Helpers.isValidNinjaApi(contents)) {
      ...
      reject(
        new Error(
          `The data from the requested ${api.type} API (League: ${league}) could not be processed because the format is invalid or the response is empty. Possible reasons: 1) Invalid league name, 2) poe.ninja is down, 3) poe.ninja changed their API structure`
        )
      )
    }
  }
  ...
  _getRequestCallsArray(league, delay) {
    ...
    for (let i = 0; i < this.apis.length; i++) {
      ...
      const method = this._requestApiData(api, league, delay * i)
  ...
  _updateCurrencyDetails(data) {
    if (Helpers.hasCurrencyDetailsData(data)) {
      ...
      if (
        !Helpers.isSameObject(this.data.CurrencyDetails, data.currencyDetails)
      ) {
  ...
  getItem(name, options) {
    ...
    return new Promise(function (resolve, reject) {
      const matches = self._getItemMatches(name, options)
  ...
  _getItemMatches(name, options) {
    if (this._hasDataForLeague(options.league)) {
      for (const type in this.data[options.league]) {
        const matches = this._getMatchesInType(type, name, options)
  ...
  _getMatchesInType(type, name, options) {
    const overview = Helpers.getOverviewByType(type, this.apis)
    let matches = []
  ...
  _getItemMatchesInType(type, name, options) {
    const itemArray = this.data[options.league][type]

    if (type === 'DeliriumOrb' && name === 'Delirium orb') {
      return itemArray
    }

    let matches = _.where(itemArray, { name })
    ...
    if (type === 'SkillGem') {
      matches = _.filter(matches, (item) => {
        return item.gemLevel === 4
      })
    }
    ...
    if (options.variantSpecified) {
      const variantMatches = _.where(matches, { variant: options.variant })

      if (variantMatches.length > 0) {
        matches = variantMatches
      } else {
        const fallbackVariantMatches = _.where(matches, {
      ...
      const nullMatches = _.where(matches, { variant: 'null' })
  ...
  _getCurrencyMatchesInType(type, name, options) {
    const currencyArray = this.data[options.league][type]
  ...
  getCurrencyDetails(name) {
    ...
    if (this.data.hasOwnProperty('CurrencyDetails')) {
      const matches = this.data.CurrencyDetails.filter(function (item) {
  ...
  save() {
    ...
    return new Promise(function (resolve, reject) {
      fs.writeFile(
        self.saveFile,
        JSON.stringify(self.data, null, 4),
        (error) => {
          if (error) {
            reject(error)
            return
